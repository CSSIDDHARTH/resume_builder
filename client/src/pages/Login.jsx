...
const Login = () => {
    ...
    return (
        <div style={{ minHeight: '100vh', display: 'flex', background: '#070b14', overflow: 'hidden' }}>
            {/* Left Panel */}
            <div className="hidden lg:flex flex-col justify-between p-12 relative overflow-hidden" style={{ width: '45%', background: 'linear-gradient(135deg, #0d1526 0%, #1e1b4b 50%, #0f172a 100%)', borderRight: '1px solid rgba(99,102,241,0.15)' }}>
                ...
                <div className="flex items-center gap-2 relative z-10">
                    ...
                    <span className="font-bold text-lg" style={{ color: '#f0f4ff' }}>Resume<span style={{ color: '#818cf8' }}>AI</span></span>
                </div>
                <div className="relative z-10">
                    <h2 className="text-4xl font-black mb-4 leading-tight" style={{ color: '#e2e8f0', letterSpacing: '-0.02em' }}>
                        Build resumes that<br /><span className="gradient-text">get you hired.</span>
                    </h2>
                    ...
                    <div className="flex -space-x-2">
                        {[11, 22, 32, 47].map(n => (
                            <img key={n} src={`https://i.pravatar.cc/32?img=${n}`}
                                className="w-8 h-8 rounded-full border-2" style={{ borderColor: '#070b14' }} alt="user" />
                        ))}
                    </div>
                    <div>
                        <p className="text-xs font-semibold" style={{ color: '#e2e8f0' }}>10,000+ resumes built</p>
                        <div className="flex gap-0.5 mt-0.5">
                            {Array.from({ length: 5 }).map((_, i) => <span key={i} style={{ color: '#f59e0b', fontSize: 10 }}>★</span>)}
            ...
            <div className="flex-1 flex items-center justify-center px-6 py-12 relative">
                ...
                <div className="w-full max-w-sm relative z-10">
                    ...
                    <div className="rounded-2xl p-8"
                        style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(99,102,241,0.2)', backdropFilter: 'blur(20px)', boxShadow: '0 0 40px rgba(99,102,241,0.1)' }}>
                        ...
                        <p className="text-sm mb-7" style={{ color: '#64748b' }}>
                            {state === "login" ? "Sign in to continue building your resume" : "Join thousands of professionals using ResumeAI"}
                        </p>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            {state !== "login" && (
                                <div className="relative">
                                    <User2 size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: '#475569' }} />
                                    <input type="text" name="name" placeholder="Full name" value={formData.name} onChange={handleChange} required style={{ paddingLeft: '2.5rem' }} />
                                </div>
                            )}
                            <div className="relative">
                                <Mail size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: '#475569' }} />
                                <input type="email" name="email" placeholder="Email address" value={formData.email} onChange={handleChange} required style={{ paddingLeft: '2.5rem' }} />
                            </div>
                            <div className="relative">
                                <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: '#475569' }} />
                                <input type="password" name="password" placeholder="Password" value={formData.password} onChange={handleChange} required style={{ paddingLeft: '2.5rem' }} />
                            </div>
                            <button type="submit" disabled={isLoading}
                                ...
                                style={{ background: isLoading ? 'rgba(99,102,241,0.4)' : 'linear-gradient(135deg, #6366f1, #a855f7)', boxShadow: isLoading ? 'none' : '0 0 20px rgba(99,102,241,0.4)', cursor: isLoading ? 'not-allowed' : 'pointer', marginTop: '0.25rem' }}
                                ...
                                {isLoading ? (
                                    ...
                                    <>{state === "login" ? "Sign In" : "Create Account"}<ArrowRight size={16} /></>
                                )}
                            </button>
                        </form>
