import {
    ...
    Share2Icon, EyeIcon, EyeOffIcon, DownloadIcon, Trophy, Save, AlertTriangle
} from "lucide-react"
...
const ResumeBuilder = () => {
    ...
    const defaultSectionOrder = [
        { id: "summary", label: "Professional Summary" }, { id: "experience", label: "Experience" },
        { id: "education", label: "Education" }, { id: "projects", label: "Projects" },
    ...
    const [resumeData, setResumeData] = useState({
        ...
        experience: [], education: [], project: [], skills: [], achievements: [], certifications: [],
    ...
    const loadExistingResume = async () => {
        try {
            ...
            if (data.resume) {
                ...
                setSectionOrder(data.resume.sectionOrder?.length ? data.resume.sectionOrder : defaultSectionOrder)
    ...
    useEffect(() => { const t = setTimeout(() => checkResumeOverflow(), 100); return () => clearTimeout(t) }, [resumeData])
    ...
    const changeResumeVisibility = async () => {
        try {
            ...
            setResumeData({ ...resumeData, public: !resumeData.public }); toast.success(data.message)
    ...
    const handleShare = () => {
        ...
        else toast.error('Share not supported on this browser.')
    }

    const tempDownload = async () => { try { window.print() } catch (error) { toast.error("Failed to download resume.") } }

    const downloadResume = async () => {
        try {
            const resume = document.getElementById("resume-preview")
            if (!resume) { toast.error("Resume preview not found."); return }
            const canvas = await html2canvas(resume, { scale: 3, useCORS: true, backgroundColor: "#fff", width: resume.scrollWidth, height: resume.scrollHeight, windowWidth: resume.scrollWidth, windowHeight: resume.scrollHeight })
            const imgData = canvas.toDataURL("image/png")
            const pdf = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" })
            const pdfWidth = 210; const imgWidth = pdfWidth; const imgHeight = (canvas.height * imgWidth) / canvas.width
            pdf.addImage(imgData, "PNG", 0, 0, imgWidth, imgHeight)
            if (isOverflowing) { const proceed = window.confirm("This resume exceeds one A4 page.\n\nContinue?"); if (!proceed) return }
            pdf.save(`${resumeData.title || "Resume"}.pdf`); toast.success("Resume downloaded successfully!")
    ...
    const saveResume = async () => {
        try {
            ...
            if (typeof resumeData.personal_info.image === 'object') delete updatedResumeData.personal_info.image
            const formData = new FormData()
            formData.append("resumeId", resumeId)
            ...
            const { data } = await api.put('/api/resumes/update', formData, { headers: { Authorization: token } })
            setResumeData(data.resume); toast.success(data.message)
        } catch (error) { toast.error("Unable to save your resume. Please try again.") }
    }
                ...
                <div className="grid lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-5">
                        <div className="rounded-2xl overflow-hidden"
                            style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(99,102,241,0.15)', backdropFilter: 'blur(12px)' }}>
                            <div className="h-0.5 w-full" style={{ background: 'rgba(255,255,255,0.05)' }}>
                                <div className="h-full transition-all duration-500"
                                    style={{ width: `${progressPct}%`, background: 'linear-gradient(90deg, #6366f1, #a855f7)' }} />
                            </div>
                            <div className="p-6">
                                <div className="flex items-center justify-between mb-5 pb-5"
                                    ...
                                    <span className="text-sm font-semibold" style={{ color: '#e2e8f0' }}>{activeSection.name}</span>
                                    <div className="flex items-center gap-1">
                                        ...
                                        <button onClick={() => setActiveSectionIndex(prev => Math.min(prev + 1, sections.length - 1))}
                                            ...
                                            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium"
                                            style={{ background: 'rgba(255,255,255,0.05)', color: activeSectionIndex === sections.length - 1 ? '#334155' : '#64748b', border: '1px solid rgba(255,255,255,0.08)', cursor: activeSectionIndex === sections.length - 1 ? 'not-allowed' : 'pointer' }}
                                ...
                                <div className="flex flex-wrap gap-2 mb-6">
                                    {sections.map((sec, i) => {
                                        const Icon = sec.icon; const active = i === activeSectionIndex
                                        return (
                                            <button key={sec.id} onClick={() => setActiveSectionIndex(i)}
                                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all"
                                                style={{ background: active ? 'rgba(99,102,241,0.18)' : 'rgba(255,255,255,0.03)', border: `1px solid ${active ? 'rgba(99,102,241,0.4)' : 'rgba(255,255,255,0.06)'}`, color: active ? '#a5b4fc' : '#475569' }}>
                                ...
                                <div className="space-y-4">
                                    ...
                                    {activeSection.id === 'summary' && <ProfessionalSummary data={resumeData.professional_summary} onChange={data => setResumeData(prev => ({ ...prev, professional_summary: data }))} setResumeData={setResumeData} />}
                                    ...
                                    {activeSection.id === 'achievements' && <Achievements data={resumeData.achievements} onChange={data => setResumeData(prev => ({ ...prev, achievements: data }))} />}
                                ...
                                <button onClick={() => { toast.promise(saveResume(), { loading: 'Saving...', success: 'Saved!', error: 'Failed to save' }) }}
                    ...
                    <div className="lg:col-span-7 max-lg:mt-4">
                        ...
                        <div className="flex items-center gap-2 justify-end mb-3">
                            {resumeData.public && (
                                <button onClick={handleShare} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-all"
                            ...
                            <button onClick={changeResumeVisibility} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-all"
                                style={{ background: resumeData.public ? 'rgba(99,102,241,0.1)' : 'rgba(255,255,255,0.04)', border: `1px solid ${resumeData.public ? 'rgba(99,102,241,0.3)' : 'rgba(255,255,255,0.08)'}`, color: resumeData.public ? '#818cf8' : '#64748b' }}
                            ...
                            <button onClick={tempDownload} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-all"
                        ...
                        <div className="rounded-2xl overflow-hidden"
                            style={{ border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 20px 60px rgba(0,0,0,0.4)' }}>
