        ...
        <>
            <style>{`
                ...
                .cta-btn {
                    ...
                    padding: 0.75rem 2rem; border-radius: 0.75rem; border: none; cursor: pointer;
                    transition: all 0.3s ease; box-shadow: 0 0 20px rgba(99, 102, 241, 0.3);
                    display: inline-flex; align-items: center; gap: 0.5rem; text-decoration: none;
                }
                .cta-btn:hover { box-shadow: 0 0 35px rgba(99, 102, 241, 0.55); transform: translateY(-2px); }
                .ghost-btn {
                    background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.12);
                    color: #94a3b8; font-weight: 500; font-size: 0.9rem;
                    padding: 0.75rem 2rem; border-radius: 0.75rem; cursor: pointer; transition: all 0.3s ease;
                    display: inline-flex; align-items: center; gap: 0.5rem; text-decoration: none;
                }
                .ghost-btn:hover { background: rgba(255,255,255,0.09); border-color: rgba(99,102,241,0.4); color: white; }
                .stat-card {
                    background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.07);
                    border-radius: 1rem; padding: 1rem 1.5rem; text-align: center;
                    backdrop-filter: blur(12px); transition: all 0.3s;
                }
                .stat-card:hover { border-color: rgba(99,102,241,0.3); box-shadow: 0 0 20px rgba(99,102,241,0.15); }
                .badge-pill {
                ...
                .scrolldown-icon { animation: float 3s ease-in-out infinite; }
            `}</style>
            ...
            <header id="home" className="relative min-h-screen flex flex-col overflow-hidden"
                ...
                {PARTICLES.map((p, i) => <Particle key={i} style={{ top: p.top, left: p.left, animationDelay: p.animationDelay }} />)}
                ...
                <nav className="nav-glass sticky top-0 z-50 w-full">
                    <div className="flex items-center justify-between max-w-7xl mx-auto px-6 py-4">
                        ...
                        <div className="hidden md:flex items-center gap-8 text-sm">
                            {['#features', '#testimonial', '#faq'].map((href, i) => (
                                <a key={href} href={href} className="transition-colors" style={{ color: '#94a3b8' }}
                                    onMouseEnter={e => e.target.style.color = '#f0f4ff'}
                                    onMouseLeave={e => e.target.style.color = '#94a3b8'}>
                                    {['Features', 'Testimonials', 'FAQ'][i]}
                                </a>
                            ))}
                        </div>
                        <div className="hidden md:flex items-center gap-3">
                            {user ? (
                                <Link to="/app" className="cta-btn"><Zap size={15} />Dashboard</Link>
                            ) : (
                                <>
                                    <Link to="/app?state=login" className="ghost-btn">Sign In</Link>
                                    <Link to="/app?state=register" className="cta-btn"><Sparkles size={14} />Get Started</Link>
                                </>
                            )}
                        </div>
                        <button className="md:hidden p-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.05)', color: '#94a3b8' }}
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center px-6 py-20 relative z-10">
                    <div className="badge-pill mb-8">
                        ...
                        AI Analysis · Career Insights · Skill Gap Detection
                    </div>
                    <h1 className="hero-gradient-text text-4xl sm:text-6xl lg:text-7xl font-black leading-tight max-w-5xl mb-6"
                        style={{ letterSpacing: '-0.02em' }}>
                        AI Resume Analysis &<br />Career Recommendations
                    </h1>
                    <p className="max-w-xl text-base sm:text-lg leading-relaxed mb-10" style={{ color: '#94a3b8' }}>
                        Upload your resume to discover skill gaps, explore tailored career paths, and find the perfect learning resources to land your next role.
                    </p>
                    <div className="flex flex-col sm:flex-row gap-4 mb-16">
                        {!user && (
                            <Link to="/app?state=register" className="cta-btn">
                                <Sparkles size={16} />Start Analyzing Free
                            </Link>
                        )}
                        <Link to="/app?state=login" className="ghost-btn">
                            <FileText size={16} />{user ? 'Go to Dashboard' : 'Try Demo'}
                        </Link>
                    </div>
                    <div className="grid grid-cols-3 gap-4 max-w-lg w-full">
                        {[
                            { num: '10k+', label: 'Resumes Analyzed' },
                            { num: '50k+', label: 'Career Insights' },
                            { num: '30 sec', label: 'Avg. Analysis Time' },
                        ].map((stat, i) => (
